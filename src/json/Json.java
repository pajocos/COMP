...
public class Json/* @bgen(jjtree) */implements JsonTreeConstants, JsonConstants {/*
																				 * @
																				 * bgen
																				 * (
																				 * jjtree
	...
	public static boolean syntaticError = false;

	private static String[] edgeStyle = { "dashed", "dotted", "solid", "bold" };

	private static String[] nodeShapes = { "box", "polygon", "ellipse", "oval",
			...
			"star", "none", "underline", "note", "tab", "folder", "box3d",
	...
	private static String[] colors = { "aliceblue", "antiquewhite",
			...
			"crimson", "cyan", "cyan1", "cyan2", "cyan3", "cyan4",
			...
			"floralwhit", "forestgreen", "gainsboro", "ghostwhite", "gold",
			"gold1", "gold2", "gold3", "gold4", "goldenrod", "goldenrod1",
			"goldenrod2", "goldenrod3", "goldenrod4", "gray", "gray0", "gray1",
			"gray10", "gray100", "gray11", "gray12", "gray13", "gray14",
			"gray15", "gray16", "gray17", "gray18", "gray19", "gray2",
			"gray20", "gray21", "gray22", "gray23", "gray24", "gray25",
			"gray26", "gray27", "gray28", "gray29", "gray3", "gray30",
			"gray31", "gray32", "gray33", "gray34", "gray35", "gray36",
			"gray37", "gray38", "gray39", "gray4", "gray40", "gray41",
			"gray42", "gray48", "gray49", "gray5", "gray50", "gray51",
			"gray52", "gray53", "gray54", "gray55", "gray56", "gray57",
			"gray58", "gray59", "gray6", "gray60", "gray61", "gray62",
			"gray63", "gray64", "gray65", "gray66", "gray67", "gray68",
			"gray69", "gray7", "gray70", "gray71", "gray72", "gray73",
			"gray74", "gray75", "gray76", "gray77", "gray78", "gray79",
			"gray8", "gray80", "gray81", "gray82", "gray83", "gray84",
			"gray85", "gray86", "gray87", "gray88", "gray89", "gray9",
			"gray90", "gray91", "gray92", "gray93", "gray94", "gray95",
			"gray96", "gray97", "gray98", "gray99", "green", "green1",
			"green2", "green3", "green4", "greenye,llow", "grey", "grey0",
			"grey1", "grey10", "grey100", "grey11", "grey12", "grey13",
			"grey14", "grey15", "grey16", "grey17", "grey18", "grey19",
			"grey2", "grey20", "grey21", "grey22", "grey23", "grey24",
			"grey25", "grey26", "grey27", "grey28", "grey29", "grey3",
			"grey30", "grey31", "grey32", "grey33", "grey34", "grey35",
			"grey36", "grey37", "grey38", "grey39", "grey4", "grey40",
			"grey41", "grey42", "grey43", "grey44", "grey45", "grey46",
			"grey47", "grey48", "grey49", "grey5", "grey50", "grey51",
			"grey52", "grey53", "grey54", "grey55", "grey56", "grey57",
			"grey58", "grey59", "grey6", "grey60", "grey61", "grey62",
			"grey63", "grey64", "grey65", "grey66", "grey67", "grey68",
			"grey69", "grey7", "grey70", "grey71", "grey72", "grey73",
			"grey74", "grey75", "grey76", "grey77", "grey78", "grey79",
			"grey8", "grey80", "grey81", "grey82", "grey83", "grey84",
			"grey85", "grey86", "grey87", "grey88", "grey89", "grey9",
			"grey90", "grey91", "grey92", "grey93", "grey94", "grey95",
			"grey96", "grey97", "grey98", "grey99", "honeydew", "honeydew1",
			"honeydew2", "honeydew3", "honeydew4", "hotpink", "hotpink",
			...
			"lightpink1", "lightpink2", "lightpink4", "lightsalmon",
			...
			"none", "oldlace", "olivedrab", "olivedrab1", "olivedrab2",
			...
			"royalblue2", "royalblue3", "royalblue4", "saddlebrown", "salmon",
			"salmon1", "salmon2", "salmon3", "salmon4", "sandybrown",
	...
	public static void main(String args[]) throws ParseException,
		...
		SimpleNode nodes = (SimpleNode) root.jjtGetChild(0); // Nó com todos os
																// nós
		SimpleNode links = (SimpleNode) root.jjtGetChild(1); // Nó com todos os
																// links
		...
		int fileCreated = createDotFile(newNodes, newLinks, args[1], args[2],
				args[3], args[4]);
		if (fileCreated == 1)
			System.out.println("\u005cnFile generated successfully!");
		else
	...
	public static ArrayNode[] processNodes(SimpleNode nodes) {
		...
		for (int i = 0; i < nodes.jjtGetNumChildren(); i++) {
			...
			an.setGroup(Integer.parseInt(nodes.jjtGetChild(i).jjtGetChild(1)
					.getNodeValue()));
			...
			an.setBeginColumn(nodes.jjtGetChild(i).jjtGetChild(0)
					.getBeginColumn());
			newNodes[i] = an;
		}
		return newNodes;
	...
	public static ArrayLink[] processLinks(SimpleNode links) {
		...
		for (int i = 0; i < links.jjtGetNumChildren(); i++) {
			...
			an.setValues(
					Integer.parseInt(links.jjtGetChild(i).jjtGetChild(0)
							.getNodeValue()),
					Integer.parseInt(links.jjtGetChild(i).jjtGetChild(1)
							.getNodeValue()),
					Integer.parseInt(links.jjtGetChild(i).jjtGetChild(2)
							.getNodeValue()),
					links.jjtGetChild(i).jjtGetChild(0).getBeginLine(), links
							.jjtGetChild(i).jjtGetChild(0).getBeginColumn());
	...
	public static boolean verifyNodes(ArrayNode[] nodes) {
		...
		for (int i = 0; i < nodes.length; i++) {
			...
			if (!tempSet.add(str)) {
				System.out.println("Duplicated name: " + str + " at line "
						+ nodes[i].getBeginLine() + " column "
						+ nodes[i].getBeginColumn());
				error = true;
			}
			...
			if (!existShape && !nodes[i].getShape().equals("")) {
				System.out.println("Unknown shape: " + nodes[i].getShape()
						+ " at line " + nodes[i].getBeginLine() + " column "
						+ nodes[i].getBeginColumn());
				error = true;
			}
		}
	...
	public static boolean verifyLinks(ArrayLink[] links, int numNodes) {
		...
		for (int i = 0; i < links.length; i++) {
			...
			if (source >= numNodes || target >= numNodes) // Verifica se os nos
															// de origem e
															// destino existem
			{
				error = true;
				System.out.println("Source or target node inexistent at line "
						+ links[i].getBeginLine() + " column "
						+ links[i].getBeginColumn());
			}
			if (source == target) // Verifica se o no de origem e de destino sao
									// o mesmo
			{
				error = true;
				System.out.println("Same source and target nodes at line "
						+ links[i].getBeginLine() + " column "
						+ links[i].getBeginColumn());
			}
			for (int j = i + 1; j < links.length; j++) // Verifica se ha
														// ligaçoes duplicadas
			{
				if (links[i].getSource() == links[j].getSource()
						&& links[i].getTarget() == links[j].getTarget()) {
					error = true;
					System.out.println("Duplicated link at line "
							+ links[i].getBeginLine() + " column "
							+ links[i].getBeginColumn());
				}
			}
			...
			if (!existStyle && !links[i].getStyle().equals("")) {
				error = true;
				System.out.println("Link style doesn't exist at line "
						+ links[i].getBeginLine() + " column "
						+ links[i].getBeginColumn());
			}
		}
	...
	public static int createDotFile(ArrayNode[] nodes, ArrayLink[] links,
			String savePath, String saveName, String type, String graphvizPath)
		...
		for (int i = 0; i < links.length; i++) {
			String line = nodes[links[i].getSource()].getName() + " -> "
					+ nodes[links[i].getTarget()].getName() + " [label = "
					+ links[i].getValue();
			if (!links[i].getStyle().equals("")) {
		...
		if (tempFolder.mkdirs() && !graphvizPath.equals("")) {
			success = gv.writeGraphToFile(gv.getGraph(gv.getDotSource(), type),
					out);
			tempFolder.delete();
		} else {
	...
	void error_skipto(int kind) throws ParseException {
		...
		try {
			...
			ParseException e = generateParseException(); // generate the
															// exception object.
			System.out.println("Error found at column "
					+ e.currentToken.beginColumn + " line "
